  ...
  class Connect
    private
     ...
     def create(filename,repo)
       curb_post("#{self.host}/api2/repos/#{repo}/file/?p=/#{filename}",{"operation"=> "create"}).body_str
     end
     ...
     def move(filename,src_repo,data)
       curb_post("#{self.host}/api2/repos/#{src_repo}/file/?p=/#{filename}",data).body_str
     end
     ...
     def copy(src_repo,data)
       curb_post("#{self.host}/api2/repos/#{src_repo}/fileops/copy/",data).body_str
     end
     ...
     def rename(old_name,repo,data)
       curb_post("#{self.host}/api2/repos/#{repo}/file/?p=/#{old_name}",data).body_str
     end
  end
end