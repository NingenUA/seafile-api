module SeafileApi
  class Connect
    private
     ...
     def delete(filename)
       curl_delete("#{self.host}/api2/repos/#{self.repo}/file/?p=/#{filename}",'').body_str
     end
     ...
     def revert(filename,commit_id)
       curl_put("#{self.host}/api2/repos/#{self.repo}/file/revert",{"commit_id" => commit_id,"p" => filename }).body_str
     end
     ...
     def create(filename)
       c = curl_new("#{self.host}/api2/repos/#{self.repo}/file/?p=/#{filename}")
     ...
     def copy(filename,dst_dir,dst_repo)
       c = curl_new("#{self.host}/api2/repos/#{self.repo}/fileops/copy/")
     ...
     def rename(old_name,new_name)
       c = curl_new("#{self.host}/api2/repos/#{self.repo}/file/?p=/#{old_name}")
