...
describe 'SeafileApi' do
  ...
  describe 'work with library' do
    before(:all) do
      @new_library = JSON.parse(SF::create_library("test_library","description","123456"))
      @default_library = JSON.parse(SF::create_default_library)
    end
    ...
    it "should create new default library" do
      expect(@default_library.to_s).to match("repo_id")
    end
    ...
    it "should get default library" do
      expect(SF::default_library).to match("repo_id")
    end
    it "should show list of libraries" do
      expect(SF::list_libraries).to match(/\[/)
    end
    it "should create sub library" do
      expect(SF::create_sub_library(@default_library["repo_id"],"test_sub_library")).to match("sub_repo_id")
    end
    it "should show list virtual libraries" do
      expect(SF::list_virtual_libraries).to match("virtual-repos")
    end
    it "should show library info" do
      expect(SF::library_info(@new_library["repo_id"])).to match("permission")
    end
    it "should show library owner" do
      expect(SF::library_owner(@new_library["repo_id"])).to match("detail")
    end
    it "should show library history" do
      expect(SF::library_history(@new_library["repo_id"])).to match("commits")
    end
    it "should show library download info" do
      expect(SF::fetch_library_info(@new_library["repo_id"])).to match("repo_id")
    end
